...
#
# NOTE: the Carnot PNG is portrait-oriented (655x381px, aspect ~1.72), so
# at icon width=2.6 it renders ~4.47 units tall. The scale below must keep
# the camera frame taller than that, with margin left over for the icon
# row in SubstanceIconsNoDifference — 0.75 gives a frame height of 6.0,
# leaving ~0.76 units of clear space above the icon's own top edge.
SUBSTANCE_ZOOM_SCALE = 0.75
...
class SubstanceIconsNoDifference(MovingCameraScene):
    def construct(self):
        ...
        row_y = min(
            carnot_icon.get_top()[1] + 0.15,
            self.camera.frame.get_top()[1] - 0.15,
        )  # clamped to the visible frame, in case the icon extends past
           # the camera's top edge (as it does at this zoom level) —
           # smaller buffer than before, pulls the row closer to the engine
        steam = make_steam_icon().scale(0.85).move_to([-0.5, row_y, 0])
        air = make_air_icon().scale(0.85).move_to([0, row_y, 0])
        fluid = make_fluid_icon().scale(0.85).move_to([0.5, row_y, 0])
...
# ---------------------------------------------------------------------------
# "Not the size of the engine, or its design." (~5.0s)
# Picks up already zoomed on the Carnot icon (same crop as
# SubstanceIconsNoDifference ends on).
#   Beat 1 (3.0s) "Not the size of the engine" — a glowing outline traces
#     around the icon's bounding box, pulses once, fades.
#   Beat 2 (2.0s) "or its design" — a few glowing pulses appear at
#     plausible internal points on the engine art (Option B: eyeballed
#     against the image, not locked to real vector geometry — the PNG has
#     no separable internal paths to trace exactly).
# ---------------------------------------------------------------------------
class SizeDesignHighlight(MovingCameraScene):
    def construct(self):
        self.camera.background_color = BG
        ...
        # --- Beat 1 (3.0s): "Not the size of the engine" ---
        # A glowing rounded-rectangle traces around the icon's bounding
        # box, pulses once for emphasis, then fades — a stand-in for
        # "highlighting the whole size of it," since the PNG has no true
        # silhouette path to trace exactly.
        outline = RoundedRectangle(
            corner_radius=0.15,
            width=carnot_icon.width * 1.08,
            height=carnot_icon.height * 1.04,
            color=GOLD,
            stroke_width=3,
        ).move_to(carnot_icon.get_center())

        self.play(Create(outline), run_time=1.2)
        self.play(
            outline.animate.set_stroke(width=5).scale(1.03),
            rate_func=there_and_back,
            run_time=1.0,
        )
        self.wait(0.3)
        self.play(FadeOut(outline), run_time=0.5)
        # Beat 1 subtotal: 1.2 + 1.0 + 0.3 + 0.5 = 3.0s

        # --- Beat 2 (2.0s): "or its design" ---
        # Small glowing pulses at a handful of plausible internal points —
        # roughly the top manifold, each cylinder bank, and the pulley/
        # belt area — eyeballed against the reference art. Staggered so
        # they don't all fire in unison.
        internal_points = [
            np.array([0, 0.63, 0]),      # top manifold area
            np.array([-0.72, 0.22, 0]),  # left cylinder bank
            np.array([0.72, 0.22, 0]),   # right cylinder bank
            np.array([0, -0.11, 0]),     # pulley / belt area
        ]
        pulse_lifecycles = []
        for i, pt in enumerate(internal_points):
            glow = Circle(radius=0.12, color=GOLD, stroke_width=0, fill_opacity=0)
            glow.move_to(pt)
            self.add(glow)
            pulse_lifecycles.append(
                Succession(
                    Wait(i * 0.12),
                    glow.animate.set_fill(opacity=0.7).scale(1.4),
                    glow.animate.set_fill(opacity=0.0).scale(1 / 1.4),
                )
            )
        self.play(AnimationGroup(*pulse_lifecycles, lag_ratio=0), run_time=2.0)
        # Beat 2 subtotal: 2.0s
        # Scene total: 3.0 + 2.0 = 5.0s


# ---------------------------------------------------------------------------
# "Just the two temperatures. Specifically, the ratio between them." (~7.0s)
# Picks up already zoomed on the Carnot icon (same crop as
# SizeDesignHighlight ends on).
#   Beat 3 (2.0s) "Just the two temperatures" — camera zooms back out to
#     reveal the full base diagram: HOT bar, engine, COLD bar, wires,
#     T_H/T_C labels, all reassembled.
#   Beat 4 (5.0s) "Specifically, the ratio between them" — the diagram
#     splits: wires fade out, the engine settles on the left, and HOT/
#     COLD detach and slide to the right, stacking close together (HOT
#     above COLD) — proximity alone reads as "ratio," no formula symbol,
#     per your call not to show one here.
# ---------------------------------------------------------------------------
class RatioSplitReveal(MovingCameraScene):
    def construct(self):
        self.camera.background_color = BG
        ...
        self.add(hot, cold, carnot_icon, top_wire, bottom_wire, th_label, tc_label)
        self.camera.frame.scale(SUBSTANCE_ZOOM_SCALE).move_to(SUBSTANCE_ZOOM_CENTER)

        # --- Beat 3 (2.0s): "Just the two temperatures" ---
        # Zoom back out to the default framing, revealing the whole
        # assembled diagram at once.
        self.play(
            self.camera.frame.animate.scale(1 / SUBSTANCE_ZOOM_SCALE).move_to(ORIGIN),
            run_time=2.0,
            rate_func=smooth,
        )

        # --- Beat 4 (5.0s): "Specifically, the ratio between them" ---
        # Wires fade first (the diagram is about to break apart), then the
        # engine settles left while HOT and COLD detach and slide right,
        # stacking close together. T_H/T_C travel with their bars since
        # they're grouped for this move.
        self.play(FadeOut(top_wire), FadeOut(bottom_wire), run_time=0.6)

        hot_group = VGroup(hot, th_label)
        cold_group = VGroup(cold, tc_label)

        engine_target = LEFT * 2.0
        hot_target = RIGHT * 2.7 + UP * 0.6
        cold_target = RIGHT * 2.7 + DOWN * 0.6

        self.play(
            carnot_icon.animate.move_to(engine_target),
            hot_group.animate.move_to(hot_target),
            cold_group.animate.move_to(cold_target),
            run_time=3.4,
            rate_func=smooth,
        )
        self.wait(1.0)
        # Beat 4 subtotal: 0.6 + 3.4 + 1.0 = 5.0s
        # Scene total: 2.0 + 5.0 = 7.0s
