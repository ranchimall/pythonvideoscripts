...
class BondingCurveExamples(Scene):
    def make_fraction(self, num_str, den_str, font_size=20, color=WHITE):
        """Builds a stacked fraction (numerator / bar / denominator), like real math notation."""
        num = Text(num_str, font_size=font_size, weight=BOLD, color=color)
        den = Text(den_str, font_size=font_size, weight=BOLD, color=color)
        bar_width = max(num.width, den.width) + 0.12
        bar = Line(LEFT * bar_width / 2, RIGHT * bar_width / 2, color=color, stroke_width=2)
        return VGroup(num, bar, den).arrange(DOWN, buff=0.06)
    ...
    def show_example(self, title, accent, x_desc, y_desc, bar_width, bar_height,
        ...
        # Price = (y/x) = (num/den) = result  -- built as real stacked fractions
        price_word = Text("Price =", weight=BOLD, font_size=19, color=accent_color)
        frac_yx = self.make_fraction("y", "x", font_size=19, color=accent_color)
        eq1 = Text("=", weight=BOLD, font_size=19, color=accent_color)
        frac_values = self.make_fraction(calc_num, calc_den, font_size=19, color=accent_color)
        eq2 = Text("=", weight=BOLD, font_size=19, color=accent_color)
        result_text = Text(price_result, weight=BOLD, font_size=19, color=accent_color)
        calc_row = VGroup(price_word, frac_yx, eq1, frac_values, eq2, result_text).arrange(RIGHT, buff=0.18)

        bottom_group = VGroup(
            values_header, line1, line2, divider, calc_label, calc_row
